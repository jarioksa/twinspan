...
#' \code{\link{predict.twinspan}}). If these two differ, the quadrat is
#' \dQuote{misclassified}. With \code{verbose=FALSE}, the function
...
#' misclassified quadrats, and the division level where the
#' misclassification ocurred and classifications diverged.
#'
#' @return
#'
...
#' \item{diverged}{The division level at which the misclassification occurred.}
#' }
#'
#' @seealso \code{\link{cut.twinspan}}, \code{\link{predict.twinspan}}.
#'
#' @examples
#'
#' data(ahti)
    ...
    function(x, verbose = TRUE)
{
    ...
    level <- x$levelmax
    div <- rep(level, length(ind))
    cl <- class
    pr <- pred
    for(lev in rev(seq_len(level-1))) {
        lim <- 2^(lev+1) - 1
    ...
    function(x, ...)
{
    ...
    if (nmiss == 0)
        ...
        cat("\n", nmiss, "misclassified quadrats and their level of divergence:\n\n")
        out <- data.frame("ind"=x$index, "Quadrat" = x$labels,
                          "Class" = x$class, "Predicted" = x$predicted,
                          "Divergence" = x$diverged)
        names(out)[1] <- ""
