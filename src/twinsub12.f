     ...
     3     COLWGT,JNAME1,JNAM,X3,X4,X5,LIND,
     4     INLEVMAX,INMMIN, EIGEN, INDICS, LIMPOS, ISEC)
...
C     score for positive group).
     ...
     1     INDPOT(NN),INDORD(NN),IZONE(MM),IY(NN),
     2     JJCOL(NN),IDAT(NDAT),INDSIG(MMS),
     3     IPICT(MMZ,MMS)
...
C     IF(ISEC.EQ.2.AND.IPUNCH.EQ.1) GO TO 21
 ...
 40   CALL WEIGHT(M,MM,N,NN,NDAT,X,Y,IIROW,IADDR,
     1     RRWT,ROWWGT,JJCOL,CCWT,COLWGT,IDAT)
 ...
 46      CONTINUE
     ...
     1        ROWWGT,CCWT,COLWGT,IADDR,IDAT)
         CALL XMAXMI(X,AXMAX,AXMIN,M)
...
C     TO DO THIS WE REQUIRE SPECIES TOTALS (IGNORING PSEUDOSPECIES WEIGHTS)
 ...
 52   CONTINUE
     ...
     1     IDAT,ICLASS,IADDR,INDPOT,JJCOL,RRWT,CCWT,COLWGT,Y,YY)
      IIIIC=IC/2+1
...
C     IN THIS CASE IIC IS ODD
 ...
 63   CALL ZONEUP(X,AXMIN,CRMIN,CRMAX,AXMAX,M,MZ,MZOUT,MZCRIT,IZONE)
     ...
     1     CUT1,CUT2,Y,YY,IIROW,ROWWGT,IADDR,IDAT)
      DO 65 J=1,N
         Y(J)=YY(J)/AXPOS-Y(J)/AXNEG
 65   CONTINUE
C---  Argument PRECIS added by P.Minchin June 1997
      CALL TOPIND(N,NN,MIND,FEEBLE,Y,INDORD,INDSIG,
     1     JJCOL,INDPOT,JNAM,NIND,LIND,PRECIS)
